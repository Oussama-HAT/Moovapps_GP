...
public class TabBaseNotationTechnique extends BaseViewExtension {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && this.taskName.equals("PreparationDuDossierDeLAO")) {
                ...
                CtlNumber baseDeNotationTechnique = ctlNumber("BaseDeNotationTechnique", linkedResource);
                viewModelItem.setValue("BaseDeNotationTechnique", baseDeNotationTechnique);
                CtlNumber noteEliminatoire = ctlNumber("NoteEliminatoire", linkedResource);
                viewModelItem.setValue("NoteEliminatoire", noteEliminatoire);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabBaseNotationTechnique.this.sysAdminContext);
                ...
                if (box.getSysname().equals("NomPrenom")) {
                    try {
                        iLinkedResource.setValue(box.getSysname(), TabBaseNotationTechnique.this.workflowModule.getUserByLogin((String)box.getSelectedKey()));
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabBaseNotationTechnique.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabBaseNotationTechnique.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabBaseNotationTechnique.this.getWorkflowInstance().createLinkedResource("BaseNotationTech_AO_Tab");
            TabBaseNotationTechnique.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabBaseNotationTechnique.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabBaseNotationTechnique.this.getWorkflowInstance().save(TabBaseNotationTechnique.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
