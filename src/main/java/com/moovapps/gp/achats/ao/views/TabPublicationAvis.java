...
public class TabPublicationAvis extends BaseViewExtension {
    ...
    public void onPrepareColumns(List list) {
        try {
            ...
            if (this.currentTaskInstance != null) {
                ...
                if (this.taskName != null && this.taskName.equals("PublicationDeLAvisDAO")) {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && this.taskName.equals("PublicationDeLAvisDAO")) {
                CtlAutocompleteList code = ctlAutocompleteList("TypePublicationAvis", linkedResource, (List<Option>)null);
                viewModelItem.setValue("TypePublicationAvis", code);
                CtlTextArea intitule = ctlTextArea("MoyenDePublication", linkedResource);
                viewModelItem.setValue("MoyenDePublication", intitule);
                CtlDate dateEnvoi = ctlDate("DateDEnvoi", linkedResource);
                viewModelItem.setValue("DateDEnvoi", dateEnvoi);
                CtlDate datePublication = ctlDate("DateDePublication", linkedResource);
                viewModelItem.setValue("DateDePublication", datePublication);
                CtlMultipleFileUpload avisLettreDEnvoi = ctlMultipleFileUpload("AvisLettreDEnvoi", linkedResource);
                viewModelItem.setValue("AvisLettreDEnvoi", avisLettreDEnvoi);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabPublicationAvis.this.sysAdminContext);
                ...
                if (box.getSysname().equals("NomPrenom")) {
                    try {
                        iLinkedResource.setValue(box.getSysname(), TabPublicationAvis.this.workflowModule.getUserByLogin((String)box.getSelectedKey()));
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabPublicationAvis.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabPublicationAvis.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabPublicationAvis.this.getWorkflowInstance().createLinkedResource("PublicationAvis_AO_Tab");
            TabPublicationAvis.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabPublicationAvis.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabPublicationAvis.this.getWorkflowInstance().save(TabPublicationAvis.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
