...
public class TabDossiersAdministratifs extends BaseViewExtension {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && this.taskName.equals("OuvertureDesDossiersAdministratifs")) {
                CtlAutocompleteList decisionCommission = ctlAutocompleteList("DecisionCommissionDA", linkedIWorkflowInstance, (List<Option>)null);
                viewModelItem.setValue("DecisionCommissionDA", decisionCommission);
                CtlDate dateCommission = ctlDate("DateDecisionCommissionDA", linkedIWorkflowInstance);
                viewModelItem.setValue("DateDecisionCommissionDA", dateCommission);
                CtlTextArea commentaire = ctlTextArea("CommentaireDossierAdministratif", linkedIWorkflowInstance);
                viewModelItem.setValue("CommentaireDossierAdministratif", commentaire);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabDossiersAdministratifs.this.sysAdminContext);
                TabDossiersAdministratifs.this.getView().refreshItems();
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabDossiersAdministratifs.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabDossiersAdministratifs.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            try {
                IWorkflowInstance linkedWorkflowInstance = TabDossiersAdministratifs.this.getWorkflowModule().createWorkflowInstance(TabDossiersAdministratifs.this.loggedOnUserContext, WorkflowsService.getWorflow("Achats", "DepotDesDossiers_1.0"), null);
                linkedWorkflowInstance.setValue("ReferenceDeLAO", TabDossiersAdministratifs.this.getWorkflowInstance().getValue("sys_Reference"));
                TabDossiersAdministratifs.this.getWorkflowInstance().addLinkedWorkflowInstance("Fournisseurs_SP", linkedWorkflowInstance);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabDossiersAdministratifs.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabDossiersAdministratifs.this.getWorkflowInstance().save(TabDossiersAdministratifs.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
