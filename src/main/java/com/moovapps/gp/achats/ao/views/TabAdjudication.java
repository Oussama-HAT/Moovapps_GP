...
public class TabAdjudication extends BaseViewExtension {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && this.taskName.equals("Adjudication")) {
                CtlAutocompleteList decisionCommission = ctlAutocompleteList("DecisionDeLaCommissionAdjudication", linkedIWorkflowInstance, (List<Option>)null);
                viewModelItem.setValue("DecisionDeLaCommissionAdjudication", decisionCommission);
                CtlTextArea commentaire = ctlTextArea("CommentaireAdjudication", linkedIWorkflowInstance);
                viewModelItem.setValue("CommentaireAdjudication", commentaire);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabAdjudication.this.sysAdminContext);
                ...
                iLinkedResource.save(TabAdjudication.this.sysAdminContext);
                TabAdjudication.this.getView().refreshItems();
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabAdjudication.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabAdjudication.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            try {
                IWorkflowInstance linkedWorkflowInstance = TabAdjudication.this.getWorkflowModule().createWorkflowInstance(TabAdjudication.this.loggedOnUserContext, WorkflowsService.getWorflow("Achats", "DepotDesDossiers_1.0"), null);
                linkedWorkflowInstance.setValue("ReferenceDeLAO", TabAdjudication.this.getWorkflowInstance().getValue("sys_Reference"));
                TabAdjudication.this.getWorkflowInstance().addLinkedWorkflowInstance("DepotDossiers_SP", linkedWorkflowInstance);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabAdjudication.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabAdjudication.this.getWorkflowInstance().save(TabAdjudication.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
