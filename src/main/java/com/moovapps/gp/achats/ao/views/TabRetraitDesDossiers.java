...
public class TabRetraitDesDossiers extends BaseViewExtension {
    ...
    public void onPrepareColumns(List list) {
        try {
            ...
            if (this.currentTaskInstance != null) {
                ...
                if (this.taskName != null && this.taskName.equals("RetraitDesDossiers")) {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && this.taskName.equals("RetraitDesDossiers")) {
                ...
                CtlDate dateDeRetrait = ctlDate("DateDeRetrait", linkedResource);
                viewModelItem.setValue("DateDeRetrait", dateDeRetrait);
                CtlTextArea lieuRetrait = ctlTextArea("LieuDeRetrait", linkedResource);
                viewModelItem.setValue("LieuDeRetrait", lieuRetrait);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabRetraitDesDossiers.this.sysAdminContext);
                ...
                if (box.getSysname().equals("NomPrenom")) {
                    try {
                        iLinkedResource.setValue(box.getSysname(), TabRetraitDesDossiers.this.workflowModule.getUserByLogin((String)box.getSelectedKey()));
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabRetraitDesDossiers.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabRetraitDesDossiers.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabRetraitDesDossiers.this.getWorkflowInstance().createLinkedResource("RetraitDossiers_AO_Tab");
            TabRetraitDesDossiers.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabRetraitDesDossiers.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabRetraitDesDossiers.this.getWorkflowInstance().save(TabRetraitDesDossiers.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
