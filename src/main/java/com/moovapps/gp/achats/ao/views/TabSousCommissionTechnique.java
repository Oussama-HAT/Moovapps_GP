...
public class TabSousCommissionTechnique extends BaseViewExtension {
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabSousCommissionTechnique.this.sysAdminContext);
                ...
                if (box.getSysname().equals("NomPrenom")) {
                    try {
                        iLinkedResource.setValue(box.getSysname(), TabSousCommissionTechnique.this.workflowModule.getUserByLogin((String)box.getSelectedKey()));
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabSousCommissionTechnique.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabSousCommissionTechnique.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabSousCommissionTechnique.this.getWorkflowInstance().createLinkedResource("SousCommissionTech_AO_Tab");
            TabSousCommissionTechnique.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabSousCommissionTechnique.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabSousCommissionTechnique.this.getWorkflowInstance().save(TabSousCommissionTechnique.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
