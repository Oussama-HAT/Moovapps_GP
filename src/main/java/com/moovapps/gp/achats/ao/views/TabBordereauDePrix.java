...
public class TabBordereauDePrix extends BaseViewExtension {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && this.taskName.equals("PreparationDuDossierDeLAO")) {
                CtlMultipleFileUpload bordereauDePrix = ctlMultipleFileUpload("BordereauDesPrix", linkedResource);
                viewModelItem.setValue("BordereauDesPrix", bordereauDePrix);
                CtlMultipleFileUpload bordereauDePrixDetailEstimatif = ctlMultipleFileUpload("BordereauDesPrixDetailEstimatif", linkedResource);
                viewModelItem.setValue("BordereauDesPrixDetailEstimatif", bordereauDePrixDetailEstimatif);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                if (box.getSysname().equals("NomPrenom")) {
                    try {
                        iLinkedResource.setValue(box.getSysname(), TabBordereauDePrix.this.workflowModule.getUserByLogin((String)box.getSelectedKey()));
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabBordereauDePrix.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabBordereauDePrix.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabBordereauDePrix.this.getWorkflowInstance().createLinkedResource("BP_AO_Tab");
            TabBordereauDePrix.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabBordereauDePrix.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabBordereauDePrix.this.getWorkflowInstance().save(TabBordereauDePrix.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
