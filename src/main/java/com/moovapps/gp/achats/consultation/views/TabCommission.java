...
public class TabCommission extends BaseViewExtension {
    ...
    public void onPrepareColumns(List list) {
        try {
            ...
            if (this.currentTaskInstance != null) {
                ...
                if (this.taskName != null && (this.taskName.equals("PreparationDeLaFicheDeConsultation") || this.taskName.equals("EnregistrementDuBonDeReception"))) {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && (this.taskName.equals("PreparationDeLaFicheDeConsultation") || this.taskName.equals("EnregistrementDuBonDeReception"))) {
                CtlAutocompleteList nomPrenom = ctlAutocompleteList("NomPrenom", linkedResource, getListUsers());
                viewModelItem.setValue("NomPrenom.fullName", nomPrenom);
                CtlAutocompleteList qualite = ctlAutocompleteList("Qualite", linkedResource, (List<Option>)null);
                viewModelItem.setValue("Qualite", qualite);
    ...
    public CtlMultipleFileUpload ctlMultipleFileUpload(String name, ILinkedResource linkedResource) {
        ...
        try {
            Collection<IAttachment> attachments = (ArrayList<IAttachment>)this.workflowModule.getAttachments((IResource)linkedResource, name);
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabCommission.this.sysAdminContext);
                ...
                if (box.getSysname().equals("NomPrenom")) {
                    try {
                        iLinkedResource.setValue(box.getSysname(), TabCommission.this.workflowModule.getUserByLogin((String)box.getSelectedKey()));
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabCommission.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabCommission.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private List<Option> getListUsers() {
        List<Option> options = new ArrayList<>();
        try {
            Collection<IUser> users = (Collection<IUser>) getDirectoryModule().getUsers(this.sysAdminContext, DirectoryService.getInternalOrganization());
            for (IUser user : users)
                options.add(new Option(user.getLogin(), user.getFullName()));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return options;
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabCommission.this.getWorkflowInstance().createLinkedResource("Commission_Tab");
            TabCommission.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabCommission.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabCommission.this.getWorkflowInstance().save(TabCommission.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
