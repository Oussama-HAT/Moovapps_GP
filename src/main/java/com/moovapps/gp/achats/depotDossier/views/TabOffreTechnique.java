...
public class TabOffreTechnique extends BaseViewExtension {
    ...
    public void onPrepareColumns(List list) {
        try {
            ...
            sysAdminContext = getWorkflowModule().getSysadminContext();
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null)
                if (this.taskName.equals("EvaluationTechnique") || this.taskName.equals("DemandeDeLeveeDesReserves") || this.taskName.equals("Evaluation")) {
                    ...
                    if (this.taskName.equals("EvaluationTechnique") || this.taskName.equals("Evaluation")) {
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabOffreTechnique.sysAdminContext);
                ...
                if (box.getSysname().equals("ConformiteDeLaPiece"))
                    if (TabOffreTechnique.this.taskName.equals("EvaluationTechnique"))
                        TabOffreTechnique.MAJDecisionCommission(TabOffreTechnique.this.getWorkflowInstance());
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabOffreTechnique.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabOffreTechnique.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabOffreTechnique.this.getWorkflowInstance().createLinkedResource("OT_Depot_Tab");
            TabOffreTechnique.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabOffreTechnique.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabOffreTechnique.this.getWorkflowInstance().save(TabOffreTechnique.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
    ...
    public static void MAJDecisionCommission(IWorkflowInstance workflowInstanceDepot) {
        try {
            String decision = "Retenu";
            String conformitePiece = null;
            Collection<ILinkedResource> linkedResources = (Collection<ILinkedResource>) workflowInstanceDepot.getLinkedResources("OT_Depot_Tab");
        ...
        if (decision.equals("Acceptée avec réserve") || decision.equals("Rejeté")) {
            workflowInstanceDepot.setValue("DecisionCommissionET", decision);
