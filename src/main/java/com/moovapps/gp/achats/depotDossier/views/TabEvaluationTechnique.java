...
public class TabEvaluationTechnique extends BaseViewExtension {
    ...
    public void onPrepareItem(ViewItem iViewItem) {
        try {
            ...
            if (this.taskName != null && this.taskName.equals("EvaluationTechnique")) {
                CtlNumber note = ctlNumber("Note", linkedResource);
                viewModelItem.setValue("Note", note);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                if (box.getSysname().equals("NomPrenom")) {
                    try {
                        iLinkedResource.setValue(box.getSysname(), TabEvaluationTechnique.this.workflowModule.getUserByLogin((String)box.getSelectedKey()));
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabEvaluationTechnique.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabEvaluationTechnique.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabEvaluationTechnique.this.getWorkflowInstance().createLinkedResource("ET_Depot_Tab");
            TabEvaluationTechnique.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabEvaluationTechnique.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabEvaluationTechnique.this.getWorkflowInstance().save(TabEvaluationTechnique.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };

    private void MAJDecisionSousCommissionTechnique() {
        try {
            Collection<ILinkedResource> linkedResourcesET = (Collection<ILinkedResource>) getWorkflowInstance().getLinkedResources("ET_Depot_Tab");
            Float noteEliminatoire = (Float)getWorkflowInstance().getValue("NoteEliminatoireTechnique");
            Float noteObtenue = Float.valueOf(0.0F), note = Float.valueOf(0.0F);
            for (ILinkedResource linkedResourceET : linkedResourcesET) {
                note = (Float)linkedResourceET.getValue("Note");
                if (note != null)
                    noteObtenue = Float.valueOf(noteObtenue.floatValue() + note.floatValue());
            }
            getWorkflowInstance().setValue("NoteObtenueTechnique", noteObtenue);
            if (noteObtenue.floatValue() > noteEliminatoire.floatValue()) {
                getWorkflowInstance().setValue("DecisionCommissionET", "Retenu");
            } else {
                getWorkflowInstance().setValue("DecisionCommissionET", "Rejeté");
            }
            TabOffreTechnique.MAJDecisionCommission(getWorkflowInstance());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
