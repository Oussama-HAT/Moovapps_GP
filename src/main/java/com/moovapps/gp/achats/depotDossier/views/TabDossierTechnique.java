...
public class TabDossierTechnique extends BaseViewExtension {
    ...
    protected ChangeListener listener = new ChangeListener() {
        public void onChange(ChangeEvent paramChangeEvent) {
            ...
            if (CtlTextArea.class.equals(object.getClass())) {
                ...
                iLinkedResource.save(TabDossierTechnique.this.sysAdminContext);
                ...
                if (box.getSysname().equals("ConformiteDeLaPiece"))
                    if (TabDossierTechnique.this.taskName.equals("OuvertureDuDossierAdministratif"))
                        TabDossierAdministratif.MAJDecisionCommission(TabDossierTechnique.this.getWorkflowInstance());
                ...
                try {
                    ...
                    for (IAttachment attachment : documents)
                        TabDossierTechnique.this.workflowModule.addAttachment((IResource)iLinkedResource, multipleFileUpload.getSysname(), attachment);
                    iLinkedResource.save(TabDossierTechnique.this.sysAdminContext);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    protected ActionListener addLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            ILinkedResource iLinkedResource = TabDossierTechnique.this.getWorkflowInstance().createLinkedResource("DT_Depot_Tab");
            TabDossierTechnique.this.getWorkflowInstance().addLinkedResource(iLinkedResource);
        }
    };

    protected ActionListener removeLine = new ActionListener() {
        public void onClick(ActionEvent paramActionEvent) {
            Iterator<CtlListView.Item> itr = TabDossierTechnique.this.getView().getListView().getSelectedItems().iterator();
            while (itr.hasNext()) {
                CtlListView.Item item = itr.next();
                CoreDocument coreDocument = (CoreDocument)item.getParam();
                TabDossierTechnique.this.getWorkflowInstance().save(TabDossierTechnique.this.getWorkflowModule().getLoggedOnUserContext());
            }
        }
    };
}
